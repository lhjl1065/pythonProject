...
def find_image(screenshot, target_image_path, threshold=0.80):
    ...
    if max_val > threshold:
        if 1569 > max_loc[0] > 1565:
            print("疲劳值为0，即将切换角色")
            return True
        if 1804 > max_loc[0] > 1800:
            print("副本失败，即将切换角色")
            return True
    return False
...
def find_image_method():
    time.sleep(3)
    target_image_path1 = r"C:\Users\Administrator\Pictures\ZEROPL.png"  # 请确保此处设置了正确的目标图像文件路径
